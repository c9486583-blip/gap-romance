...
const IdentityVerification = () => {
  ...
  const startVerification = async () => {
    ...
    setLoading(true);
    try {
      // Mark step as verification pending before redirect
      await supabase
        .from("profiles")
        .update({ onboarding_step: ONBOARDING_STEPS.VERIFICATION_PENDING } as any)
        .eq("user_id", user.id);
      ...
      if (error || !data?.url) {
        // Revert step if verification couldn't start
        await supabase
          .from("profiles")
          .update({ onboarding_step: ONBOARDING_STEPS.PROFILE_COMPLETE } as any)
          .eq("user_id", user.id);
        toast({
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <OnboardingProgress currentStep={5} totalSteps={6} stepLabel="Identity Verification" />
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg text-center">
        ...
        <p className="text-muted-foreground mb-8 max-w-md mx-auto">
          GapRomance requires all members to verify their identity to keep the platform safe and
        ...
        <Button variant="hero" size="lg" className="w-full max-w-sm mx-auto" onClick={startVerification} disabled={loading}>
          {loading ? <><Loader2 className="mr-2 w-5 h-5 animate-spin" /> Starting Verification...</> :
